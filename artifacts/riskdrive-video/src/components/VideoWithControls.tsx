...
/**
 * Returns true when the page is being rendered by the Replit video export
 * pipeline (which loads the video as a top-level document and sets
 * window.startRecording before React mounts).
 *
 * Controls are hidden during export so the recorded frame stays clean.
 * All other contexts — iframe preview, direct published URL, local dev —
 * receive the full control bar.
 */
function isExportRender(): boolean {
  if (typeof window === 'undefined') return false;
  // URL-param escape hatch: add ?export=1 to manually trigger export mode
  const params = new URLSearchParams(window.location.search);
  if (params.get('export') === '1') return true;
  // The Replit export pipeline registers startRecording before page load
...
export default function VideoWithControls() {
  ...
  // Export path: no props so recording markers fire correctly, no control bar
  if (exportMode) return <VideoTemplate />;
