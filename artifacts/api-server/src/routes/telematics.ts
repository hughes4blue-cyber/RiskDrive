...
router.get("/telematics/connections", async (req, res): Promise<void> => {
  ...
  const rows = facilityIdRaw
    ? await db
        .select()
        .from(telematicsConnectionsTable)
        .where(eq(telematicsConnectionsTable.facilityId, parseId(facilityIdRaw as string)))
        .orderBy(desc(telematicsConnectionsTable.createdAt))
    : await db
        .select()
        .from(telematicsConnectionsTable)
        .orderBy(desc(telematicsConnectionsTable.createdAt));
...
router.delete("/telematics/connections/:id", async (req, res): Promise<void> => {
  const id = parseId(req.params.id);
  const [row] = await db
    .delete(telematicsConnectionsTable)
    .where(eq(telematicsConnectionsTable.id, id))
    .returning();
  if (!row) {
    res.status(404).json({ error: "Connection not found" });
...
router.post("/telematics/connections/:id/sync", async (req, res): Promise<void> => {
  ...
  try {
    ...
    req.log.error({ err: message, connectionId: id }, "Telematics sync failed");
    await db
...
router.get("/telematics/events", async (req, res): Promise<void> => {
  ...
  const rows = facilityIdRaw
    ? await base
        .where(eq(vehiclesTable.facilityId, parseId(facilityIdRaw as string)))
        .orderBy(desc(telematicsEventsTable.timestamp))
        .limit(limit)
